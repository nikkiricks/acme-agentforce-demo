...
| Acme board dashboard (LWC) | `force-app/main/default/lwc/agenticExperienceBoard/` | Acme's C-suite → board view of the agentic rollout: 4 KPI tiles, hand-rolled SVG trend, D/W/M toggle, two-sides pipeline strip — see `docs/dashboard-architecture.md` |
| Customer pipeline view (LWC) | `force-app/main/default/lwc/customerPipelineDashboard/` | the product Acme ships — one customer's pipeline at customer altitude, kept for Q&A |
| Board data services + tests | `force-app/main/default/classes/AgenticExperienceBoardController*.cls`, `CustomerPipelineController*.cls` | pluggable insight layer: one live CRM query today, Data Cloud / Tableau Next tomorrow |
